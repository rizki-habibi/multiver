...
const hasMultiverConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "Multiver");
};

const getMultiverEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "Multiver") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getMultiverEntry(config);
...
// POST - Apply Multiver config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing Multiver entry or append
    const idx = config.findIndex((e) => e.name === "Multiver");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove Multiver entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "Multiver");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "Multiver removed from Copilot config",
    });
  } catch (error) {
