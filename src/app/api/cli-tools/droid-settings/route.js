...
// Check if settings has Multiver customModels
const hasMultiverConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:Multiver"));
};
...
// POST - Update Multiver customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing Multiver configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:Multiver"));
...
// DELETE - Remove Multiver customModels only (keep other settings)
