...
// Check if settings has Multiver config
const hasMultiverConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["Multiver"];
};

// Read per-agent models.json and return current model id (without "Multiver/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["Multiver"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasMultiver: hasMultiverConfig(settings),
...
// POST - Update Multiver settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `Multiver/${model}`;

    // Remove all old Multiver/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("Multiver/"))
    ...
    // Add fresh Multiver models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`Multiver/${m}`] = {};
    });

    // Remove old Multiver model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("Multiver/")) {
    ...
    // Update models.providers.Multiver with all models
    settings.models.providers["Multiver"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `Multiver/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove Multiver settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove Multiver from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["Multiver"];
    ...
    // Remove Multiver models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("Multiver/"));
    ...
    // Reset agents.defaults.model.primary if it uses Multiver
    if (settings.agents?.defaults?.model?.primary?.startsWith("Multiver/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "Multiver settings removed successfully",
    });
  } catch (error) {
