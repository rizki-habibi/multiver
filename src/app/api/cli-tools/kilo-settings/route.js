...
const hasMultiverConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["Multiver"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("Multiver");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasMultiver: hasMultiverConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "Multiver", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "Multiver settings removed from Kilo Code" });
  } catch (error) {
