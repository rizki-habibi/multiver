...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-Multiver.env");
};
...
export async function GET() {
  ...
  const hasMultiver = hasMultiverConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_Multiver_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile Multiver",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["Multiver"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_Multiver_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "Multiver configuration removed from jcode",
    });
  } catch (error) {
