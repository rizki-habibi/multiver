...
const hasMultiverConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["Multiver"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["Multiver"];
    ...
    return NextResponse.json({
      ...
      configPath: getConfigPath(),
        opencode: {
          ...
          activeModel: config?.model?.startsWith("Multiver/") ? config.model.replace(/^Multiver\//, "") : null,
...
// POST - Apply Multiver as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_Multiver";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing Multiver provider entry and its models
    const existingProvider = config.provider["Multiver"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["Multiver"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `Multiver/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `Multiver/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("Multiver/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove Multiver provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["Multiver"]?.models) {
      delete config.provider["Multiver"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["Multiver"].models).length === 0) {
        delete config.provider["Multiver"];
        if (config.model?.startsWith("Multiver/")) delete config.model;
      } else if (config.model === `Multiver/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["Multiver"].models);
        config.model = `Multiver/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire Multiver provider
      if (config.provider) delete config.provider["Multiver"];
      if (config.model?.startsWith("Multiver/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("Multiver/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "Multiver settings removed from OpenCode",
    });
  } catch (error) {
