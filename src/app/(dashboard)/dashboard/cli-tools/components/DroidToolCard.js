...
export default function DroidToolCard({
  ...
  const currentBaseUrl = droidStatus?.settings?.customModels?.find((m) => m.id?.startsWith("custom:Multiver"))?.baseUrl || "";

  const getConfigStatus = () => {
    if (!droidStatus?.installed) return null;
    // Check for any Multiver model entry (support multi-model: custom:Multiver-0, custom:Multiver-1, ...)
    const currentConfig = droidStatus.settings?.customModels?.find(m => m.id?.startsWith("custom:Multiver"));
  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      const existingModels = (droidStatus.settings?.customModels || [])
        .filter(m => m.id?.startsWith("custom:Multiver"))
        .sort((a, b) => (a.index || 0) - (b.index || 0))
        .map(m => m.model);
      if (existingModels.length > 0) {
        setModelList(existingModels);
      } else {
        // Legacy: single model stored as custom:Multiver-0
        const legacy = droidStatus.settings?.customModels?.find(m => m.id === "custom:Multiver-0");
        if (legacy?.model) {
          ...
          {!checkingDroid && droidStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {droidStatus?.settings?.customModels?.find(m => m.id?.startsWith("custom:Multiver"))?.baseUrl && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!droidStatus?.hasMultiver} loading={restoring}>
