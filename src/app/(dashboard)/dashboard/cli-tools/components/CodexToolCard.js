...
export default function CodexToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_Multiver for localhost if no key, otherwise use selected key
  ...
  const getManualConfigs = () => {
    ...
    const configContent = `# Multiver Configuration for Codex CLI
model = "${selectedModel}"
model_provider = "Multiver"

[model_providers.Multiver]
name = "Multiver"
base_url = "${getEffectiveBaseUrl()}"
wire_api = "responses"

[model_providers.Multiver.http_headers]
