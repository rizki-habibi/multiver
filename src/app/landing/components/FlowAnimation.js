...
export default function FlowAnimation() {
  ...
  return (
    <div className="mt-16 w-full max-w-4xl relative h-[360px] hidden md:flex items-center justify-center animate-[float_6s_ease-in-out_infinite]">
      {/* Multiver Hub - Center */}
      <div className="relative z-20 w-32 h-32 rounded-full bg-[#161b22] border-2 border-[#58A6FF] shadow-[0_0_40px_rgba(88,166,255,0.3)] flex flex-col items-center justify-center gap-1 group cursor-pointer hover:scale-105 transition-transform duration-500">
        <span className="material-symbols-outlined text-4xl text-[#58A6FF]">
          hub
        </span>
        <span className="text-xs font-bold text-white tracking-widest uppercase">
          Multiver
        </span>
        <div className="absolute inset-0 rounded-full border border-[#58A6FF]/30 animate-ping opacity-20"></div>
      </div>
      ...
      {/* SVG Lines from CLI to Multiver */}
      <svg
      ...
      {/* SVG Lines from Multiver to Providers */}
      <svg
      ...
      >
        <path
          ...
          fill="none"
          stroke={activeFlow === 0 ? "#58A6FF" : "rgb(75, 85, 99)"}
        ...
        <path
          ...
          fill="none"
          stroke={activeFlow === 1 ? "#58A6FF" : "rgb(75, 85, 99)"}
        ...
        <path
          ...
          fill="none"
          stroke={activeFlow === 2 ? "#58A6FF" : "rgb(75, 85, 99)"}
        ...
        <path
          ...
          fill="none"
          stroke={activeFlow === 3 ? "#58A6FF" : "rgb(75, 85, 99)"}
      ...
      <div className="absolute right-0 top-0 bottom-0 flex flex-col justify-between py-6">
        {PROVIDERS.map((provider, idx) => (
          <div
            key={provider.id}
            className={`px-4 py-2 rounded-lg ${provider.color} ${provider.textColor} flex items-center justify-center font-bold text-xs shadow-lg hover:scale-110 transition-all cursor-help min-w-[140px] ${activeFlow === idx ? "ring-4 ring-[#58A6FF]/50 scale-110" : ""
              }`}
            title={provider.name}
          >
            {provider.name}
      ...
      {/* Mobile fallback */}
      <div className="md:hidden mt-8 w-full p-4 rounded-lg bg-[#161b22] border border-[#30363d]">
