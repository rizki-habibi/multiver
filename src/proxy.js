...
export default async function proxy(request) {
  ...
  // mv_mimo_login cookie — route handlers and this proxy run in separate
  ...
  // valid dashboard session: a forged mv_mimo_login cookie (client-controlled
  ...
  const hasSessionCookie = cookies.includes("mv_mimo_login=");
  ...
  if (hasSessionCookie && !(await isAuthenticated(request))) {
    ...
    headers.append("Set-Cookie", "mv_mimo_login=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0");
