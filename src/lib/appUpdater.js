...
// Collect PIDs of all Multiver-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from Multiver install dir or wrapper cli.js
        const isAppProcess = lower.includes("Multiver") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("Multiver") ||
