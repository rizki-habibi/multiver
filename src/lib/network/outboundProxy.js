...
export function applyOutboundProxyEnv(
  ...
  if (!enabled) {
    if (process.env.MULTIVER_PROXY_MANAGED === "1") {
      delete process.env.HTTP_PROXY;
      delete process.env.HTTPS_PROXY;
      delete process.env.ALL_PROXY;
      delete process.env.NO_PROXY;
      delete process.env.MULTIVER_PROXY_MANAGED;
      delete process.env.MULTIVER_PROXY_URL;
      delete process.env.MULTIVER_NO_PROXY;
    }
    return;
  }
  ...
  const wasManaged = process.env.MULTIVER_PROXY_MANAGED === "1";
  let managed = false;
  ...
  if (proxyUrl) {
    ...
    if (validated) {
      ...
      process.env.MULTIVER_PROXY_URL = validated;
      managed = true;
    }
  }
  ...
  if (noProxy) {
    ...
    process.env.MULTIVER_NO_PROXY = noProxy;
    managed = true;
  }

  if (managed) {
    process.env.MULTIVER_PROXY_MANAGED = "1";
  } else if (wasManaged) {
