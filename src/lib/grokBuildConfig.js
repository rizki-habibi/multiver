...
const previousDefaultRegExp = /^# Multiver-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# Multiver-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via Multiver gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# Multiver-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# Multiver-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}
