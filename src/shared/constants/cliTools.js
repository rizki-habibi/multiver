...
export const CLI_TOOLS = {
  ...
  copilot: {
    ...
    configType: "guide",
    docsUrl: "https://marketplace.visualstudio.com/items?itemName=hotrungnhan.Multiver-for-github-copilot",
    guideSteps: [
      {
        step: 1,
        title: "Install Extension",
        desc: "In VS Code, open Extensions (Ctrl+Shift+X or Cmd+Shift+X), search for 'Multiver for Github Copilot' and click Install.",
      },
      {
        step: 2,
        title: "Configure Server",
        desc: "Press Cmd+Shift+P (or Ctrl+Shift+P), run 'Multiver: Configure Server', then enter your Server URL and API Key:",
        value: "{{baseUrl}}",
        copyable: true,
      },
      {
  ...
  amp: {
    ...
    guideSteps: [
      ...
      { step: 5, title: "Add Shorthands", desc: "Map Amp shorthand names such as g25p or cs45 to Multiver aliases in your local config." },
    ],
    codeBlock: {
      language: "bash",
...
# cs45 -> cc/claude-sonnet-4-5-20250929`,
  ...
  jcode: {
    ...
    notes: [
      ...
      {
        type: "info",
        text: "Configure Multiver as an OpenAI-compatible provider to route all jcode requests through Multiver's optimization layer."
      },
      {
        type: "warning",
  ...
  "grok-build": {
    ...
    notes: [
      {
        type: "info",
        text: "Grok Build uses ~/.grok/config.toml. Multiver writes a [model.Multiver] custom model and sets it as the default.",
      },
      {
        type: "info",
        text: "After Apply, run grok (or /model Multiver) to use the routed model. Switch back anytime with /model grok-build.",
      },
      {
        type: "warning",
...
devin --version`,
  ...
  opendesign: {
    ...
    guideSteps: [
      ...
      { step: 2, title: "No config needed", desc: "OpenDesign runs inside your host agent and uses its model config. If the host already routes through Multiver, /opendesign traffic does too." },
...
# Fetch and follow .opencode/INSTALL.md from the repo`,
  ...
  pi: {
    ...
    notes: [
      {
        type: "info",
        text: "Pi uses ~/.pi/agent/models.json. Multiver is configured under providers.Multiver as an OpenAI-compatible endpoint.",
      },
    ],
  },
  omp: {
    ...
    notes: [
      {
        type: "info",
        text: "Oh My Pi uses ~/.omp/agent/models.yml and agent.db. Multiver is configured with proxy discovery so all models appear automatically under /model.",
      },
    ],
  },
  crush: {
    ...
    notes: [
      {
        type: "info",
        text: "Crush uses ~/.config/crush/crush.json. Multiver registers as an openai-compat provider.",
      },
    ],
  },
  forge: {
    ...
    notes: [
      {
        type: "info",
        text: "ForgeCode uses ~/.forge/config.toml. Multiver updates the [openai] section with your baseUrl, apiKey, and model.",
      },
    ],
  },
  smelt: {
  ...
  codewhale: {
    ...
    notes: [
      {
        type: "info",
        text: "CodeWhale uses ~/.codewhale/config.toml. Multiver configures the [openai] provider with your base_url, api_key, and model.",
      },
    ],
  },
  // HIDDEN: gemini-cli
