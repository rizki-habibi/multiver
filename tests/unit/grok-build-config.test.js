...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("Multiver");
    ...
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "Multiver-general-purpose",
      explore: "Multiver-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\/.multiver\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\/.multiver-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\/.multiver-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# Multiver-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model/.multiver-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("Multiver-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model/.multiver-");
    expect(reset).not.toContain("Multiver-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("Multiver-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("Multiver-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("Multiver-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("Multiver-explore");
    expect(getGrokSubagentSlot("plan")).toBe("Multiver-plan");
