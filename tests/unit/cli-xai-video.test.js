/**
 * Tests for the `Multiver xai video` CLI command (cli/src/cli/commands/xaiVideo.js)
 *
 * Uses a real local HTTP server standing in for the Multiver gateway + video CDN.
    ...
    ({ server } = await startServer((req, res) => {
      if (req.method === "POST" && req.url === "/v1/videos/generations") {
        ...
        req.on("end", () => {
          ...
          res.writeHead(200, { "Content-Type": "application/json", "x-Multiver-connection-id": "conn-42" });
