// Guards the deduped Antigravity OAuth client: credentials now come from env,
// not hardcoded literals (GitHub Push Protection compliance).
import { describe, it, expect } from "vitest";

describe("antigravity oauth client (deduped, env-driven)", () => {
  it("shared source exposes clientId/clientSecret keys", async () => {
    const { ANTIGRAVITY_OAUTH_CLIENT } = await import("../../open-sse/providers/shared.js");
    expect(ANTIGRAVITY_OAUTH_CLIENT).toHaveProperty("clientId");
    expect(ANTIGRAVITY_OAUTH_CLIENT).toHaveProperty("clientSecret");
  });
  ...
  it("registry transport keeps clientId/clientSecret", async () => {
    ...
    expect(ag.transport).toHaveProperty("clientId");
    expect(ag.transport).toHaveProperty("clientSecret");
  });

  it("google client shared by gemini + gemini-cli", async () => {
    const { GOOGLE_OAUTH_CLIENT } = await import("../../open-sse/providers/shared.js");
    expect(GOOGLE_OAUTH_CLIENT).toHaveProperty("clientId");
    expect(GOOGLE_OAUTH_CLIENT).toHaveProperty("clientSecret");
