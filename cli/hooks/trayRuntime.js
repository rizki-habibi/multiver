...
function cleanupLegacySystray({ silent = false } = {}) {
  // 1) Runtime dir: ~/.multiver/runtime/node_modules/systray (or %APPDATA% on Win)
  // 2) npm global nested: <npm_prefix>/node_modules/multiver/node_modules/systray
  ...
  for (const dir of targets) {
    if (fs.existsSync(dir)) {
      try {
        ...
        if (!silent) console.log(`[multiver][runtime] removed legacy systray: ${dir}`);
      } catch (e) {
        if (!silent) console.warn(`[multiver][runtime] failed to remove ${dir}: ${e.message}`);
      }
    }
  }
}
...
function chmodSystrayBin({ silent = false } = {}) {
  ...
  try {
    ...
    if (!silent) console.warn(`[multiver][runtime] chmod tray bin failed: ${e.message}`);
  }
}
