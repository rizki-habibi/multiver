...
async function buildCodexHeader() {
  ...
  const { installed, hasMultiver, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasMultiver, settings } = result.data;
  ...
  // Extract Multiver custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:multiver-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract Multiver provider config
  const provider = settings?.models?.providers?.["multiver"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("multiver/") ? primary.replace("multiver/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasMultiver, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use Multiver\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
