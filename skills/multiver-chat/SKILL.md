---
name: Multiver-chat
description: Chat / code generation via Multiver using OpenAI /v1/chat/completions or Anthropic /v1/messages format with streaming + auto-fallback combos. Use when the user wants to ask an LLM, generate code, summarize text, or run prompts through Multiver.
---
...
- `POST $MULTIVER_URL/v1/chat/completions` — OpenAI format
- `POST $MULTIVER_URL/v1/messages` — Anthropic format

## Discover

```bash
curl $MULTIVER_URL/v1/models | jq '.data[].id'
# Per-model metadata (contextWindow, params)
curl "$MULTIVER_URL/v1/models/info?id=openai/gpt-4o"
```
...
```bash
curl -X POST $MULTIVER_URL/v1/chat/completions \
...
const client = new OpenAI({ baseURL: `${process.env.MULTIVER_URL}/v1`, apiKey: process.env.MULTIVER_KEY });
...
```bash
curl -X POST $MULTIVER_URL/v1/messages \
  -H "Authorization: Bearer $MULTIVER_KEY" \
